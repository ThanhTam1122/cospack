...
class SpecialCapacity(Base):
    ...
    HANMA48002 = Column("HANMA48002", DECIMAL(8, 0), nullable=True) #todo not null。 出荷日にして、運送会社コードとともに複合プライマリーキーにしてください。

    # 3. 最大重量（kg） - Max Weight (DECIMAL(10,0))
    HANMA48003 = Column("HANMA48003", DECIMAL(10, 0), nullable=True) #todo not null。　こちらが限度才数です

    # 4. 才数換算重量（kg） - Weight per Volume (DECIMAL(3,0))
    HANMA48004 = Column("HANMA48004", DECIMAL(10, 0), nullable=True) #todo not null。　こちらが最大重量です
